...
async function downloaded_models (){
  const{ recommendedTier}=await getSystemInfo();
  const response= await ollama.list();
  const local_models=response.models;
  const model=MODEL_TIERS[recommendedTier];
  if(!local_models.some(m=>m.name===model)){
    try{
